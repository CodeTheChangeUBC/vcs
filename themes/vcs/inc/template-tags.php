 ...
 */
  ...
  * @return bool
  */
 function vcs_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'vcs_categories' ) ) ) {
 		...
 		set_transient( 'vcs_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so vcs_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so vcs_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in vcs_categorized_blog.
  */
 function vcs_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'vcs_categories' );
 }
 add_action( 'edit_category', 'vcs_category_transient_flusher' );
 add_action( 'save_post',     'vcs_category_transient_flusher' );
  ...
  */
 function vcs_comment_list( $comment, $args, $depth ) {
  ...
  */
 function vcs_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
